...
class _ChatDetailsState extends State<ChatDetails> {
  ...
  // socket
  final IO.Socket socket = IO.io('http://192.168.1.70:5500',
      IO.OptionBuilder().setTransports(['websocket']).build());

  _sendMessageThroughSocket() {
    print("Sending message through socket");
    // print("$loginedUser $id ${_textController.text}");
    ChatAPI().sendMessage(conversationId, id, _textController.text);
  ...
  _connectSocket() {
    _getId();
    print("SOcket init");
    IO.Socket socket = IO.io('http://192.168.1.70:5500',
        IO.OptionBuilder().setTransports(['websocket']).build());
    ...
    socket.on('new-msg', (data) {
      print("Message received through socket iiii");
      print(data);
  ...
  Widget build(BuildContext context) {
    ...
    OtherUser = args['receiverId'];
    id = args['senderId'];
    conversationId = args['conversationId'];
    Map ids = args['ids'];
    id = ids['senderId'];
    OtherUser = ids['receiverId'];

    List chat = args['chatMessages'];
